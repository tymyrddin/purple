...
Misconfigurations recur because the surface is large and the [defaults are not always safe](shared-responsibility.md). A public storage bucket. An
...
not a network in the classical sense; it is a collection of API endpoints, [identity-bound permissions](identity-collapse.md), and configuration
...
than reactively. [Monitoring for unintended exposure](cloud-detection.md), with the assumption that exposure happens through deployment
