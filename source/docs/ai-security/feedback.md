...
applied to decisions: blocked, escalated, cleared, false positive. Those labels are worth shaping to anyone
with time and patience. Some attackers have both.
...
- [Risk scoring pipelines](../audits/supportive/risk-scoring.md) where outcome feedback is automatically incorporated into the next training run.
...
and produces an updated model. The concrete state change is the [decision boundary](decision.md) itself, which shifts
...
the boundary towards their attack. The drift is not uniform; it moves in the direction the attacker
shaped it.
...
systematically towards the no-action side, through volume and calibrated inputs, widens the no-action
...
## Training pipelines are lightly governed
...
whether their clearance decisions were sound.
...
The specific field to watch is the outcome label on a reviewed decision event in the labelling queue.
...
## Training signal corruption is slow to surface
...
## Trainable systems and durability
...
Feedback poisoning never shows up in a single event; it shows up across retraining cycles, as the boundary
moves. Every measure here is a way of making that movement visible and its cause traceable.

Seeing the movement means watching the model over time rather than at a point. [Monitoring decision
distributions across retraining cycles](../audits/supportive/continuous-monitoring.md), against a baseline,
surfaces the gradual drift that point-in-time accuracy hides; and version-controlling deployed models so their
boundaries can be compared across versions turns a silent update into an auditable change, one where a shift in
how a specific input population is classified can actually be seen.

Seeing the movement is only half of it. The other half is asking what moved it. Reviewing the training signal
sources periodically, with attention to whether any one input population is pushing the boundary in a consistent
