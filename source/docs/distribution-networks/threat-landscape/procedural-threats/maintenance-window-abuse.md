...
Threats that exploit planned maintenance windows: using a legitimate maintenance period as cover for unauthorised work,
hiding unauthorised changes within scheduled changes, or abusing the reduced oversight that maintenance windows provide.
...
Maintenance windows at Stedin are announced well in advance and are coordinated through the Bedrijfsvoering
...
activity and its completion. What windows are legitimate follows [Stedin's maintenance announcement rhythms, procurement schedules, and the
...
For instance, a scheduled maintenance activity at Stedin might require opening a switchpoint to isolate a section of
...
An insider (a Stedin contractor or employee) with access to the maintenance window is particularly dangerous, because
...
Contractors make up roughly 18 per cent of Stedin's workforce and perform much of the field maintenance, working
...
external attacker would need to either impersonate Stedin maintenance personnel (which requires knowledge of the
procedures and access to the site) or coordinate with an inside accomplice.

Stedin's werkplan and bedieningsplan are documented in advance, so an attacker can study them before the maintenance
...
unauthorised work.
    ...
    T-7 days: Werkplan published
      • Stedin publishes maintenance schedule to customer portal
    ...
    If Stedin investigates after unexpected RTU behaviour:
...
Authorised maintenance at Stedin often requires configuration changes: updating SIPROTEC or SEL relay settings,
...
For instance, a maintenance activity at Stedin might authorise changing a relay's time delay from 100 milliseconds to
...
for SIPROTEC, AcSELerator QuickSet for SEL, or similar). Contractors who perform Stedin's protection relay maintenance
have this knowledge and access, making them a particular risk.

## Unauthorised network access during maintenance

Maintenance windows at Stedin often require granting temporary access to systems that are normally restricted. A
contractor might need access to an engineering workstation to load relay settings, or might need physical access
to a Stedin substation to replace equipment. An attacker could use a maintenance window as justification for accessing a
system, then use that access to plant malicious software or gather information.

For instance, a contractor might request access to a Stedin engineering workstation during a maintenance window to
update SIPROTEC or SEL relay firmware. The operator grants access, expecting the contractor to update one relay.
The contractor could use this access to install malware on the workstation, to compromise Stedin's settings database, or
to steal a copy of the network model from Smallworld.

Alternately, a contractor with physical access to a Stedin substation could plant test equipment (a device that monitors
...
system access logs, and physical inspection of equipment after Stedin's maintenance.

## Observable traces

What maintenance window abuse might look like: work orders that don't match the actual changes made, switching plans
that differ from the executed switching, configuration changes outside the scope of the maintenance, unauthorised test
equipment connected to the network.

The first evidence is a discrepancy between Stedin's work plan and the actual work performed. The work plan specifies
...
For switching operations, Stedin's bedieningsplan specifies what switches will be operated. The operator or technician
...
physically visible. Malware installation on a Stedin engineering workstation would be detected through endpoint
...
Stedin's work plan authorises access to one relay but the access logs show the contractor connected to five relays, that
is a deviation worth investigating.

A pattern of unusual changes across multiple Stedin maintenance windows could indicate systematic abuse. If several
