...
Threats that work by changing system configuration: modifying SCADA settings, altering protection relay thresholds,
changing engineering parameters, corrupting the network model, or altering alarm thresholds to cause false operations or
mask events.
...
Stedin's SCADA system (e-terracontrol suite: e-terracontrol for SCADA/EMS and e-terra distribution for DMS) maintains
...
The deployment of new e-terracontrol configuration follows Stedin's change-control process: the configuration is built
...
trails. Detecting unauthorised modifications turns on [Stedin's change cadence and approval processes](../../operating-context/operations-and-cadence/operational-procedures-and-change.md): when and how
changes are authorised, who performs them, and what documentation trails they leave.

## Protection relay settings compromise

Stedin's protection relays (inferred as SIPROTEC 5 or SEL families like SEL-451, not independently confirmed) are configured through engineering tools (DIGSI 5
...
An attacker who can modify the relay settings would target specific thresholds. For instance, changing an
...
The engineering tools used to configure relays (DIGSI 5 for SIPROTEC, AcSELerator QuickSet for SEL), Stedin's SCADA
system (e-terracontrol), and the network model (Smallworld with Lovion integration) typically run on restricted
...
Who has access to these tools is a key part of understanding the attack surface. Stedin's own engineers have access,
and contractors performing relay maintenance and configuration work also have access during maintenance windows,
under [how Stedin vets and grants access to contractors](../../operating-context/operations-and-cadence/contractor-management.md)
and [when that access is authorised](../../operating-context/operations-and-cadence/operational-procedures-and-change.md).

Stedin's change-control process typically requires that all configuration changes be made by authorised personnel
...
Stedin's network model (maintained in Smallworld GIS with Lovion integration for network documentation) is the single
...
An attacker who can modify the network model could cause e-terracontrol to send commands to the wrong devices. For
...
Substation B would actually isolate it at Substation A. The operator would see in their e-terracontrol display that the
intended load has been shed, and would have no way to know that the wrong load was actually shed.

More subtly, an attacker could modify the network model to add phantom devices or remove real ones. A phantom
device would be sent commands by e-terracontrol and the operator would see confirmation messages, but nothing would
actually happen at the field. A removed device would be sent commands that would fail to execute, and the operator would
see a communication error. Over time, if many devices are removed from the model, the operator's view of the
...
through documented change processes. Changes to the model are typically made by Stedin's GIS engineers and require the
...
Stedin's e-terracontrol SCADA system is configured to generate alarms when certain conditions occur: frequency
deviations, voltage violations, communications failures, temperature alarms at power equipment. These alarms are visual
and sometimes audible, and trigger operator attention. Alarms are also logged in Stedin's event journal and are used by
analytics tools to detect emerging faults.

An attacker who can modify alarm thresholds in e-terracontrol could cause alarms to not trigger when they should (making
a fault appear normal), or to trigger too often (causing alarm fatigue, where operators learn to ignore alarms and might
miss a real fault). For instance, changing a voltage-alarm threshold from 0.9 per-unit to 0.8 per-unit would prevent an
...
Alarm configuration is typically stored in e-terracontrol's configuration database and is deployed to the live
...
Stedin maintains a baseline of all system settings: SIPROTEC and SEL relay protection settings, e-terracontrol SCADA
configuration, Smallworld network model, alarm thresholds, device communication parameters. When maintenance is
...
during a routine baseline comparison at Stedin. If an operator periodically connects to each relay and performs a Read
...
foundational to understanding what unauthorised changes would appear as, and follows [how Stedin verifies and
documents baseline settings](../../operating-context/operations-and-cadence/maintenance-philosophy.md).
...
activities). The challenge is that legitimate maintenance generates the same kinds of records: Stedin engineers
