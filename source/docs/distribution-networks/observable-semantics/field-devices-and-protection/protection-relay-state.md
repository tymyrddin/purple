...
Each protection relay is configured with specific settings: thresholds for overcurrent, overvoltage,
...
are accessed through engineering tools (DIGSI 5 for SIPROTEC, AcSELerator QuickSet for SEL). The operator maintains a baseline
...
Normal relay operation means the settings stored in the relay match the baseline stored in the engineering system.
...
timestamp, a user ID, a work order, and often an [as-found-and-as-left record](../engineering-and-change/maintenance-window-signatures.md) documenting the old and new settings.

The baseline-maintenance practice is so fundamental that it is assumed in the operational procedures and in the
...
Each relay continuously monitors electrical quantities (current, voltage, frequency) and records the instant when
...
begins tripping in response to a specific sequence of events (for instance, every time voltage briefly dips below 90
per cent of nominal), and that sequence never occurs in normal operation, the relay may be running conditional
...
timestamped, versioned (through Git or a similar system), and often backed up. When an engineer loads a relay
...
usually stored in the tool's session history or in Windows event logs on the engineering workstation. An unexpected
connection to a relay outside documented maintenance windows is a red flag. A connection that results in a settings
write (a download to the relay) without a corresponding work order is particularly suspicious. If the engineering
...
show a connection at 03:00 but the storingsdienst (fault-duty rota) was handling an emergency at that relay (a
...
Protection relays are paired with circuit breakers or recloser devices that actually open the circuit
...
delay for signal propagation and mechanical actuation, tens to hundreds of milliseconds). A mismatch between
