...
Compliance substitutes for the trade-off decision. A compliance framework says encryption at rest is required. The checkbox is checked. Nobody asked whether the encryption implementation causes enough performance degradation that the service disables it under peak load, or re-enables a weaker algorithm silently, or routes around it in ways that preserve the checkbox while removing the protection. Compliance describes what the system should have. It does not describe whether the system has it under the conditions that matter.
