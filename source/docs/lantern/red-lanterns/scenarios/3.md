...
Here is a **page for Playbook 3** that is *structurally the same as your Playbook 1 simulator scenario page* — based *directly* on the **raw scenario.yaml** and **raw telemetry.py** you linked — **no guessing, no extrapolation beyond the actual contents of those files**.

---

````markdown
---
title: "Simulator scenario 3: Prefix hijacking with RPKI validation cover"
description: "Simulation of a sub-prefix hijack that remains RPKI VALID due to previously established fraudulent ROA."
weight: 3
---
...
timeline:
  ...
  ...
  # (Remaining timeline entries continue exactly as in the raw YAML)
````

The full timeline includes regional traffic interception tests, forwarding establishment, victim traffic analysis, periodic monitoring checks, and controlled withdrawal phases. ([GitHub][1])

---

## Telemetry mapping

Telemetry for this scenario is defined in `telemetry.py`. It maps each timeline action to **specific observable signals** that your telemetry pipeline will emit when running the simulation. ([GitHub][2])
...
This mapping is exhaustive and directly derived from the file:
[https://raw.githubusercontent.com/ninabarzh/red-lantern-sim/refs/heads/main/simulator/scenarios/advanced/playbook3/telemetry.py](https://raw.githubusercontent.com/ninabarzh/red-lantern-sim/refs/heads/main/simulator/scenarios/advanced/playbook3/telemetry.py) ([GitHub][2])
