...
## Different from normal hijacking
...
Forwarding maintains service, which reduces detection. Monitoring is usually outage-centric, not focused on traffic path changes. As long as services remain available, investigation priority is low.
...
**Intent**
Withdraw hijack announcement in controlled manner that minimises attribution and preserves operational security for future operations, appearing like routine network maintenance rather than detected attack.
...
"Our operations team was testing RPKI deployment and mistakenly created ROA for incorrect prefix due to spreadsheet error. When we discovered the error, we immediately withdrew any announcements and corrected the ROA. We apologise for any confusion this caused."
...
Most hijacks are detected during announcement or during active phase, not during withdrawal. Clean withdrawal often goes unnoticed because victim organisation is focused on "service is restored, crisis over" rather than forensic analysis of what happened.
...
Node 1.3.3.1 "Long-term hijacking for espionage" from the hijacking tree requires accepting high attribution risk. Most operations target medium duration.
