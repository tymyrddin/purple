...
A model is a mental representation of how a system works. Everyone has them. Many are incomplete, partially wrong, 
or anchored to a state the system was in some time ago.

Some common examples in security:

"The network perimeter protects us." This model [predates cloud-native architectures](../../systems-architecture/architecture-as-model.md), remote work, and SaaS-heavy 
...
Models are not lies. They are simplifications. The problem arises when they are [treated as facts](for-defence.md), especially by people 
...
[Fixing the symptom without correcting the model](applying-sem.md) is a characteristic failure mode. Permissions get tightened after a 
