...
A Montessori insight about materials is that they work best when concrete, self-correcting, and designed to isolate a single concept or skill so that learners can engage with it directly without requiring interpretation from an instructor.

For adult security learning, this translates to exercises that are close enough to real conditions to be meaningful, [safe enough to fail in without consequence](../../crucible/index.rst), and designed so that the feedback comes from the exercise itself rather than from someone evaluating the attempt.
...
Configuration challenges: a deliberately misconfigured environment for learners to audit, diagnose, and correct. The challenge is in finding all the problems, not just the obvious ones. Extensions ask learners to explain how each misconfiguration could be exploited and what the detection approach would be.

Design problems: given a set of requirements and constraints, design a security control or detection approach. There is no single correct answer. The exercise surfaces the assumptions the learner is making and the trade-offs they are prioritising. [Peer review](reflection.md) makes the different approaches visible and produces genuine discussion.
