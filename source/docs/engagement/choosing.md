...
In [problem-solving terms](../foundations/problem-solving/in-security.md), a problem is the difference between a perceived current 
state and a desired state. That definition has a practical implication for engagement choice: the right engagement 
cannot be chosen before both states are established. An organisation that jumps directly to 
...
desired state explicit rather than leaving it implicit and therefore untestable. A long-standing observation applies here: 
...
a small facilitated change process that brings the real problems into the room rather than the sanitised versions.
...
If the gap is primarily in the emotional or political layer, which [the three-layer frame describes](../foundations/problem-solving/in-security.md) as people not following a process they know exists, or findings not being acted on despite being clearly documented, then a technical engagement is unlikely to close it. The engagement that fits best is one that examines the conditions: why the procedure is not being followed, who has the authority to change the incentive structure, and what a follow-up choreography could surface about the real constraints.
...
This engagement may update the picture of the current state and the desired state, but its main yield is paths for making the necessary changes.
